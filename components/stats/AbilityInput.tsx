...
import type { AbilityConfig, BarrierAilmentType, AbilityBuffPattern } from '@/types';
...
export default function AbilityInput({ ability, onChange }: Props) {
  ...
  const updateConvert = (ailment: BarrierAilmentType, pattern: AbilityBuffPattern | 'none') => {
    const next = { ...ability };
    // 一度削除
    next.convertAilments = next.convertAilments.filter((c) => c.ailment !== ailment);
    if (pattern !== 'none') {
  ...
  return (
    <div className="space-y-4">
        ...
        <table className="min-w-full text-xs border-collapse">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-700">
              <th className="px-2 py-1 text-left font-medium text-gray-500">結界異常</th>
              <th className="px-2 py-1 text-left font-medium text-gray-500">バフ変換先</th>
              <th className="px-2 py-1 text-center font-medium text-gray-500">無効化</th>
            </tr>
          </thead>
          <tbody>
            {AILMENTS.map((a) => {
              const conversion = ability.convertAilments.find((c) => c.ailment === a);
              const isNullified = ability.nullifyAilments.includes(a);

              return (
                <tr key={a} className="border-b border-gray-100 dark:border-gray-800/50">
                  <td className="px-2 py-2 font-medium">{a}</td>
                  <td className="px-2 py-2">
                    <select
                      value={conversion?.pattern ?? 'none'}
                      onChange={(e) => updateConvert(a, e.target.value as any)}
      ...
      <p className="text-[10px] text-gray-500 italic leading-relaxed">
        ※バフ変換対象の異常は自動的にデバフ効果も無効化されます。<br />
