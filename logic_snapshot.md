...
- **対象**: 敵の陽防・陰防のみ。
- **保存場所**: `DamageBonus.persistentEnemyYangDefDebuff` / `persistentEnemyYinDefDebuff`（0 以下の整数、optional）。
- **バレット追加効果には存在しない**。スキル・攻撃前効果で付与された状態を初期値として UI から手動入力する。

### 8.2 非FB時の計算

通常R1（能力補正込み）に永続デバフ段階を加算してから倍率を決定する。  
合算値が −10 以内は通常式、−11 以下は **減衰式** を適用。
...
その後 R2 倍率・異常デバフ倍率を乗算する（大結界はさらに別枠乗算）。

### 8.3 FB時の計算

R1/R2 はリセットされるが、永続デバフは**FB補正（÷4）とは別枠で乗算**される。  
FB中の永続デバフは減衰なし（通常の `÷(1 + 0.3 × |p|)` を適用）。

```
敵防御 = baseDef × (1/4)_FB × ÷(1+0.3×|p|)_永続 × GB倍率
```

例: 永続デバフ −2、大結界なし → `÷4 × ÷1.6 = ÷6.4`

### 8.4 貫通弾

貫通弾は Rank 補正（永続デバフ含む）を無視する。異常デバフのみ適用。

### 8.5 実装コード対応

```
getPersistentCombinedDefMult(r1WithAbility, persistentStages)  → 非FB・R1+永続の合算倍率
getAtkDefSpdMultiplier(persistentStages)                       → FB中の永続倍率（通常式）
```
