...
| `'弾性弾'` | - | 相手の攻撃バフに応じてCRI率アップ（未実装）。 |
| `'爆破弾'` | - | 相手の防御バフに応じてCRI率アップ（未実装）。 |
| `'精密弾'` | - | 相手の速力バフに応じてCRI率アップ（未実装）。 |
...
### `BuffStages`
全バフ項目の段階を保持するオブジェクト。
- 各項目に対し `R1` (通常バフ/デバフ: -10〜10) と `R2` (特殊バフ) が存在します。
- **自身バフ**: R2 は 0〜10 の範囲（強化のみ）。
- **敵デバフ**: R2 は -10〜0 の範囲（弱体化のみ）。以下の項目が含まれます：
  - `enemyYangDefR2`, `enemyYinDefR2`
  - `enemyEvasionR2`
  - `enemyCriDefR2`, `enemyCriEvasionR2` (追加)
- **Combinedバフ**: プレイヤーのバフ値と敵のデバフ値を個別に合算（R1はR1同士、R2はR2同士）し、計算式内でそれぞれ -10〜10 または 0〜10 の範囲にクランプされます。
...
### `BoostPattern` (定数)
- 代表的な型: `1-3-1`, `1-1-3`, `2-2-1` 等。
- バリデーション規則: `X-Y-Z` 形式、合計が 5。

## 7. シミュレーション設定 (SimulationConfig)
シミュレーションの全入力を保持する構造です。
- `activeBulletCount`: 使用するバレットの段数 (1〜6)。ブーストレベルと構成から自動計算される。
- (その他の既存フィールド...)
