...
- **使用段数の制御**: `SimulationConfig.activeBulletCount` に基づき、指定された段数（ID）を超えるバレットはシミュレーションおよび静的計算から完全に除外されます。除外された段はダメージを与えず、追加効果やブレイクも発生しません。
- **ブースト制御**:
  - UI上で「ブースト段階(0b-3b)」と「ブースト構成(例: 1-3-1)」を設定することで、`activeBulletCount` が自動計算される。
  - 0b: 常に 1段目のみ発動。
  - 1b〜3b: 構成（X-Y-Z）に基づき、各段階で追加される段数を累積する。
  - 不正な構成が入力された場合は、バリデーションエラーとしてシミュレーションが停止する。
...
- **フルブレイク (FB)**: 結界をすべて割り切ると発生。
  - **デバフリセット**: 発生した瞬間に、敵の全バフ（Rank1 および Rank2 の防御・回避・CRI関連）がすべて 0 にリセットされる。
  - **防御固定**: FB中の攻撃は、敵の防御デバフ（Rank1）を強制的に -10 段階として計算する。
- **表示仕様**:
  - UI上のバフ段階表示は、Rank1 と Rank2 を合算せず、`R1/R2` 形式（例：`-10/-2`）で個別に表示する。
  - フルブレイク中の敵防御表示は、計算の実態に合わせて `-10/0 (FB)` と表記する。
...
- **シミュレーション統合**: `useSimulation.ts` 内で、選択された絵札の効果（ステータス加算、条件付きバフ、属性・弾種補正）を入力データに統合した後、`runSimulation` を呼び出す。

## 4. useStoryCards.ts (絵札検索フック)
絵札データのフィルタリングを担当します。
- **名前検索**: 部分一致による絞り込み。
- **タグ検索**: 効果の種類（陽攻UP、星属性UP等）による絞り込み。

## 5. stats.ts (ステータス計算)
ステータス計算の共通ロジックを保持します。
- `calculateEffectiveStatMult`: 基礎値、蓄積、バフ、異常補正を統合した最終数値を算出。
